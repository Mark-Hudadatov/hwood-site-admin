...
import { getOrderTypeConfig } from '../../lib/OrderTypes';
...
export const OrderTypeTag: React.FC<OrderTypeTagProps> = ({
  ...
  const dotColor = brand === 'skylum' ? COLORS.skylum : COLORS.brand;
  ...
  return (
    <span style={{
      ...
      {t.label}
      {brand === 'skylum' && (
        <span style={{ fontSize: 9, color: COLORS.skylum, fontWeight: 800, letterSpacing: '.15em' }}>· SKYLUM</span>
      )}
    </span>
  );
};
