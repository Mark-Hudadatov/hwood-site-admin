...
export const ContactPage: React.FC = () => {
  ...
  useEffect(() => {
    getCompanyInfo().then(d => { if (d) setInfo(d); });
  }, []);
  ...
  // Contact detail items (from DB with fallback placeholders)
  const contactItems = [
    {
      icon: Phone,
    ...
    },
    {
      icon: Phone,
      label: 'WhatsApp',
      value: info?.whatsapp || '+972 50 890-0000',
      href: `https://wa.me/${(info?.whatsapp || '+972508900000').replace(/[\s+]/g, '')}`,
      green: true,
    },
    {
      icon: Mail,
    ...
    },
    {
      ...
      value: info?.openingHours || (lang === 'he' ? 'א׳–ה׳ · 9:00 – 18:00 IST' : 'Sun–Thu · 9:00 – 18:00 IST'),
    },
  ];
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: 40 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 20, padding: 40, boxShadow: '0 12px 40px -16px rgba(0,0,0,.12)' }}>
            {done ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400, gap: 20, textAlign: 'center' }}>
                ...
                <a href="https://wa.me/972501234567" target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '13px 22px', borderRadius: 999, background: '#25D366', color: '#fff', fontSize: 13, fontWeight: 700, textDecoration: 'none' }}>
                  Open WhatsApp
                </a>
              </div>
            ) : (
          ...
          <aside style={{ display: 'flex', flexDirection: 'column', gap: 20, paddingTop: 32 }}>
                  ...
                  <div>
                    ...
                    {item.href ? (
                      <a href={item.href} target={item.href.startsWith('http') ? '_blank' : undefined} rel="noopener noreferrer"
                        style={{ fontSize: 14, fontWeight: 600, color: item.green ? '#16a34a' : '#0a0a0a', textDecoration: 'none' }}>
                        {item.value}
                      </a>
                    ) : (
            ...
            <div style={{ background: '#f0fdf4', borderRadius: 16, padding: 24, border: '1px solid #bbf7d0' }}>
              ...
              <a href="https://wa.me/972501234567" target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '11px 20px', borderRadius: 99, background: '#25D366', color: '#fff', fontSize: 13, fontWeight: 700, textDecoration: 'none' }}>
                WhatsApp · {info?.whatsapp || '+972 50 890-0000'}
              </a>
            </div>
            ...
            <div style={{ padding: 20, borderRadius: 12, border: '1px solid #e5e5e5', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 12.5, color: '#737373', fontWeight: 300, margin: 0, lineHeight: 1.5 }}>
                  {lang === 'he'
                    ? 'ציין "Skylum" בנושא ונעביר לצוות המתאים.'
