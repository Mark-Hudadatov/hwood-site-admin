 ...
 * ProductPage is ONLY for browse-and-order services.
 */
...
import {
  Product, ProductCategory, Subservice, Service,
  ProductConfiguration, ConfigOptionType,
...
const PRODUCT_FALLBACK = `data:image/svg+xml,${encodeURIComponent(
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 600" fill="none"><rect width="800" height="600" fill="#1a1a1a"/><g transform="translate(360, 260)" stroke="#ffffff" stroke-width="2.5" fill="none" opacity="0.4"><rect x="0" y="5" width="80" height="12" rx="2"/><rect x="0" y="22" width="80" height="12" rx="2"/><rect x="0" y="39" width="80" height="12" rx="2"/></g></svg>`
)}`;
...
const ConfiguratorOption: React.FC<ConfiguratorOptionProps> = ({ option, selectedValue, onSelect }) => {
          ...
          {option.values.map((value) => (
            <button
              ...
              className={`w-10 h-10 rounded-full ring-2 ring-offset-2 transition-all ${
                selectedValue === value.slug ? 'ring-brand' : 'ring-transparent hover:ring-neutral-200'
              }`}
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-3">
        <span className="text-meta font-medium text-neutral-900 uppercase tracking-wide">{option.name}</span>
        {option.description && <Info className="w-4 h-4 text-neutral-300" />}
      </div>
      <div className="flex flex-wrap gap-3">
        {option.values.map((value) => (
          <button
...
export const ProductPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setProduct(product);
        setActiveView(product.modelUrl && product.has3DView ? '3d' : 0);
  ...
  const getQuoteUrl = () => {
    ...
    if (product)  params.set('productTitle', product.title);
  ...
  return (
    <div className="min-h-screen w-full bg-white">
      <div className="max-w-[1600px] mx-auto px-4 md:px-16 py-3 md:py-8">
        ...
        {/* Breadcrumb */}
        <nav className="flex items-center gap-2 text-[10px] md:text-meta text-neutral-500 mb-3 md:mb-8 flex-wrap">
        ...
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-1.5 text-xs md:text-base text-neutral-500 hover:text-brand mb-3 md:mb-6 transition-colors"
        >
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-12 xl:gap-16">
          ...
          <div className="w-full lg:w-3/5">
            <div className="lg:sticky lg:top-4">
              <h1 className="text-lg md:text-display-sm font-medium text-neutral-900 mb-1 md:mb-2">
                {product.title}
              </h1>
              {product.subtitle && (
                  ...
                  <>
                    ...
                    {has3D && (
                      <button
                        ...
                        className="absolute top-4 right-4 bg-black/60 text-white text-xs px-3 py-1.5 rounded-full flex items-center gap-2 hover:bg-black/80 transition-colors"
                      >
                        <Rotate3d className="w-3.5 h-3.5" />
                        Back to 3D
                      </button>
                      ...
                      {allImages.map((img, idx) => (
                        <button
                          ...
                          className={`flex-shrink-0 w-14 h-14 rounded-lg overflow-hidden transition-all border-2 ${
                            activeView === idx ? 'border-white' : 'border-transparent opacity-70 hover:opacity-100'
                          }`}
                        >
            ...
            {configuration && configuration.options.length > 0 ? (
              ...
              <div className="text-center py-6 bg-neutral-50 rounded-xl mb-10">
                <p className="text-meta text-neutral-500">
                  Configuration options not available for this product.
                </p>
              </div>
            )}
